...
Obj.GLOBAL_Title              = 'All Third-Order Spherical Harmonics Components of Weighting 1';
Obj.GLOBAL_Comment            = '';
Obj.GLOBAL_Origin             = '';
...
for harmonic_index = 1:16
    fnm = zeros(16,1);
    fnm(harmonic_index) = 1;
    ...
    %         fnm(harm_idx) = fnm(harm_idx) * scale_factor;
    %         harm_idx = harm_idx + 1;
    %     end
    % end
    
    n = Obj.SH.n(harmonic_index); % order
    m = Obj.SH.m(harmonic_index); % degree

    % correct rotations up to 3rd order
    if m > 0
    ...
    % figure;
    % AKp(real(F)', 'x7', 'az', sg(:,1), 'el', sg(:,2), 'cb', 0, 'sph_proc', 'tri', 'coord', 2, 'hp_view', [45 15])
    
    save("Active Acoustics Review/Polar Patterns/third_order_sh_unity_at_pos_"+harmonic_index+".mat", '-struct', 'Obj');
end